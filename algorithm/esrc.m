...
% Created by H.Kasai on July 06, 2017
   ...
   for i =1:size(TrainSet.X,2)
       TrainSet.X(:,i) = TrainSet.X(:,i)/norm(TrainSet.X(:,i));
   end
   for i =1:size(TestSet.X,2)
       TestSet.X(:,i) = TestSet.X(:,i)/norm(TestSet.X(:,i));
   end 
    % extract options
    ...
    for i = 1 : test_num
        ...
        % calculate sparse code
            ...
            P = (combined_X'*combined_X+lambda*eye(size(combined_X,2)))\combined_X';
            x0 = P*y;            
